...
class PPOContinuousSystem(RLBaseSystem):
    def __init__(
        ...
        self.value_module = TensorDictModule(
            value_network, in_keys=["observation"], out_keys=["state_value"]
        )
        ...
        self.policy = TensorDictSequential(
            self.mean_module, self.std_module, self.action_sampler
        )
        self.loss_module = TensorDictModule(
            PPOContinuousLoss(
                **(self.cfg.system.losses if "losses" in self.cfg.system else {})
            ),
            in_keys=[
                "mean_action",
                "mean_action_old",
    ...
    def training_step(self, batch, batch_idx):
        ...
        replay_buffer = ReplayBuffer(
            storage=LazyTensorStorage(
                max_size=n_rollouts * max_traj_len, device=self.device
            ),
        ...
        for i_roll in range(n_rollouts):
            with torch.no_grad():
                episode_data = self.env.rollout(
                    max_traj_len, policy=self.policy, auto_cast_to_device=True
                )
        ...
        if normalize_advantages:
            replay_buffer["advantage"] = (
                replay_buffer["advantage"] - replay_buffer["advantage"].mean()
            ) / (replay_buffer["advantage"].std() + 1e-8)
        cum_rw /= n_rollouts
        nbatches = math.ceil(
            epochs_per_episode * len(replay_buffer) // self.cfg.data.batch_size
        )
    ...
    def configure_optimizers(self):
        ...
        optimizer_bl = optimizer_class(
            self.value_module.module.parameters(), **cfg_optimizer.args
        )

        if "scheduler" in self.cfg.system:
            scheduler_class = getattr(
                optim.lr_scheduler, self.cfg.system.scheduler.name
            )
            scheduler_policy = scheduler_class(
                optimizer, **self.cfg.system.scheduler.args
            )
            scheduler_value = scheduler_class(
                optimizer_bl, **self.cfg.system.scheduler.args
            )
            return (
                {
                    "optimizer": optimizer,
                ...
                {"optimizer": optimizer_bl, "lr_scheduler": scheduler_value},
            )
