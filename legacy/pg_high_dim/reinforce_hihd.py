...
def reinforce(env: gym.Env):
    ...
    for episode in range(NUMBER_EPISODES):
        ...
        while not done:
            ...
            with torch.no_grad():
                probs_policy = pg_net(
                    torch.tensor(x_st, dtype=torch.float32, device=device).unsqueeze(0)
                )
        ...
        states_torch = torch.from_numpy(np.vstack([[st] for st in states_list])).to(
        ...
        act_prob = torch.gather(
            pg_net(states_torch), 1, actions_torch.unsqueeze(1)
        ).squeeze(1)
        ...
        if (episode + 1) % 10 == 0:
            ...
            print(
                "[{} Episode]\t-->\treward> {}\tloss> {}\t".format(
                    episode + 1, meanrw, loss.item()
                )
            )
