...
class ReinforceLoss:
    def __call__(
        self, action_probs: torch.Tensor, action: torch.Tensor, G: torch.Tensor
    ) -> torch.Tensor:
...
class BellmanDelta:
    ...
    def __call__(
        ...
        target_value = reward + self.gamma * self.state_value_network(
            next_observation
        ) * (1 - done.to(dtype=torch.float32))
...
class ReinforceContinuousWithActorCriticLoss(ReinforceContinuousLoss):
    ...
    def __call__(  # type: ignore[override]
        ...
        reinforce_loss = self._reinforce_loss(
            mean_action, std_action, action, delta.detach(), gamma_cumm
        )
...
class ReinforceContinuousLogVarLoss:
    def __call__(
        ...
        # dist = torch.distributions.Normal(mean_action, std_action)
        dist = NormalLogVar(
            mean_action, logvar_action, maxlogvar_action, minlogvar_action
        )
...
class PPOLoss:
    ...
    def __call__(
        ...
        action_prob = torch.gather(
            actions_probs, dim=-1, index=action.argmax(keepdim=True, dim=-1)
        )
        action_prob_old = torch.gather(
            actions_probs_old, dim=-1, index=action.argmax(keepdim=True, dim=-1)
        )
        ...
        critic_loss = torch.sum(
            (state_value_target - state_value) ** 2, dim=-1, keepdim=True
        )
