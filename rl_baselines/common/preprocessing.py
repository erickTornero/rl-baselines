...
class DQNPreprocessing0(Preprocessing):
    ...
    def __call__(
        self, rgb_tensor: torch.Tensor, last_frame: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class DQNPreprocessing(Preprocessing):
    def __init__(
        ...
        self.transform = transforms.Compose(
            [LuminanceTransform(), ReescaleTransform(out_size, interpolation)]
        )
...
class ReescaleTransform(nn.Module):
    ...
    def __call__(self, gray_image: torch.Tensor) -> torch.Tensor:
        if gray_image.ndim == 2:
            return F.resize(
                gray_image.unsqueeze(0), self.out_size, self.interpolation
            ).squeeze(0)
        else:
...
if __name__ == "__main__":
    ...
    # cv2.imshow("Reinforce Discrete", processed[0].numpy())
    k = cv2.waitKey(1)
    ts = 0
    done = False
    cum_reward = 0
    ...
    while not done:
        ...
        cv2.imshow("Reinforce Discrete", out.numpy())
        k = cv2.waitKey(30)
