...
from .losses import DDPGCriticLoss, DDPGPolicyLoss, QTargetEstimatorContinuous
from .sampler import ActionContinuousClamper, ContinuousExplorationDDPGSampler
...
class DDPGSystem(RLBaseSystem):
    def __init__(
        ...
        self.soft_update_policy = SoftUpdate(
            policy_network, policy_target_network, **cfg.system.soft_update
        )
    ...
    def get_optimizer_from_cfg(
        self, cfg_optimizer, model: nn.Module
    ) -> torch.optim.optimizer.Optimizer:
        optimizer_class = getattr(optim, cfg_optimizer.name)
        optimizer = optimizer_class(model.parameters(), **cfg_optimizer.args)
        return optimizer
    ...
    def configure_optimizers(self):
        ...
        optimizer_critic = self.get_optimizer_from_cfg(
            cfg_optimizers.critic, self.qnetwork
        )
    ...
    def from_config(cls, config: Union[str, OmegaConf]) -> DDPGSystem:
        ...
        init_final_linear_layer(
            state_action_network, -amplitude_last_layer, amplitude_last_layer
        )
        init_final_linear_layer(
