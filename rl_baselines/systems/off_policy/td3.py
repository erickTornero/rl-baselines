...
class TD3System(RLBaseSystem):
    def __init__(
        ...
        self.policy = TensorDictSequential(
            TensorDictModule(
                policy_network, in_keys=["observation"], out_keys=["action"]
            ),
            self.clamper,
        )
        """
        ...
        self.policy_explore = TensorDictSequential(
            self.policy, self.action_exploration_sampler, self.clamper
        )
        self.target_estimator = TensorDictModule(
            ...
            in_keys=[("next", "reward"), ("next", "observation"), ("next", "done")],
        ...
        self.soft_update_policy = SoftUpdate(
            policy_network, policy_network_target, **cfg.system.soft_update
        )
    ...
    def on_fit_start(self):
        ...
        self.memory_replay = ReplayBuffer(
            storage=LazyTensorStorage(
                max_size=self.cfg.data.replay_buffer_size, device=self.device
            ),
        ...
        self.target_estimator.module.action_spec = (
            self.target_estimator.module.action_spec.to(self.device)
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict_with_envname({"Episode Reward": cum_rw})

    # TODO, improve this
    def get_optimizer_from_cfg(
        self, cfg_optimizer, *models: nn.Module
        ...
        optimizer = optimizer_class(params, **cfg_optimizer.args)
        return optimizer

    def configure_optimizers(self):
        cfg_optimizers = self.cfg.system.optimizer

        optimizer_policy = self.get_optimizer_from_cfg(
            cfg_optimizers.policy, self.policy.module
        )

        optimizer_critic = self.get_optimizer_from_cfg(
            cfg_optimizers.critic, self.qnetwork_1, self.qnetwork_2
        )
    ...
    def from_config(cls, config: Union[str, OmegaConf]) -> TD3System:
        ...
        if config.system.exploration.type == "normal":
            noise_process = NormalNoise(
                **config.system.exploration.args, size=get_env_action_dim(env)
            )
        else:
            raise NotImplementedError(
                f"noise process <{config.system.exploration.type}> not supported"
            )

        if config.system.target_action_noise.type == "normal-clamped":
            target_noise_process = NormalClampedNoise(
                **config.system.target_action_noise.args, size=get_env_action_dim(env)
            )
        else:
            raise NotImplementedError(
                f"noise process <{config.system.target_action_noise.type}> not supported for target"
            )

        # initialize last layers according to the paper
        if (
            "from_pixels" in config.system.environment
            and config.system.environment.from_pixels
        ):
            # for pixels case
            amplitude_last_layer = 3e-4
        else:
            amplitude_last_layer = 3e-3
        init_final_linear_layer(
            state_action_network_1, -amplitude_last_layer, amplitude_last_layer
        )
        init_final_linear_layer(
            state_action_network_2, -amplitude_last_layer, amplitude_last_layer
        )
        init_final_linear_layer(
            policy_network, -amplitude_last_layer, amplitude_last_layer
        )
