...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume", dest="resume", type=str, default=None, required=False
    )
    parser.add_argument(
        "--save-video", dest="save_video", action="store_true", default=False
    )
    ...
    if args.train:
        ...
        logger = [
            ...
            CSVLogger(save_dir=model.working_dir, name="csv_logs"),
        ]
        ckpt_callback = ModelCheckpoint(
            dirpath=model.get_absolute_path("ckpts"),
            filename="epoch-{epoch:02d}-Episode Reward-{"
            + config.system.environment.name
            + "/Episode Reward:.2f}",
            mode="max",
    ...
    trainer = pl.Trainer(
        max_epochs=config.trainer.max_episodes, callbacks=callbacks, logger=logger
    )
    if args.train:
