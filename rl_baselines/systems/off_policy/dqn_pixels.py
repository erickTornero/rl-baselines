...
class DQNDiscretePixelsSystem(RLBaseSystem):
    def __init__(
        ...
        self.action_values_module = TensorDictSequential(
            ...
            TensorDictModule(
                qvalues_network, in_keys=["stack_proc"], out_keys=["action_values"]
            ),
        )
        ...
        self.qsampler_module = TensorDictModule(
            QPolicySampler(self.action_spec, return_onehot=True),
            in_keys=["action_values"],
            out_keys=["action", "Qvalue"],
        )
        ...
        self.policy_explore = TensorDictSequential(
            self.action_values_module, self.egreedy_sampler_module, self.firing_module
        )
        self.policy = TensorDictSequential(
            self.action_values_module, self.qsampler_module, self.firing_module
        )
        self.target_estimator = TensorDictSequential(
            ...
            TensorDictModule(
                ...
                in_keys=[("next", "reward"), ("next", "stack_proc"), ("next", "done")],
                out_keys=["Qtarget"],
            ),
        )

        self.update_target = self.load_update_networks(
            qvalues_network, qvalues_network_target, self.cfg.system.update_networks
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        for t in range(max_episode_length):
            ...
            if (
                self.agent_selection_counter % self.cfg.system.grad_update_frequency
                == 0
    ...
    def configure_optimizers(self):
        ...
        optimizer = optimizer_class(
            self.action_values_module.module.parameters(), **cfg_optimizer.args
        )
        return optimizer
    ...
    def from_config(cls, config: Union[str, OmegaConf]) -> DQNDiscretePixelsSystem:
        ...
        qnetwork = RLBaseSystem.load_network_from_cfg(
            cfg.system.network, 0, get_env_action_dim(env)
        )
