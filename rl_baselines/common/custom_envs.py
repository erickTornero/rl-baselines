...
def parse_env_cfg(
    ...
    **kwargs,
    ...
    transforms = None
    using_observation = "from_pixels" not in kwargs or not kwargs["from_pixels"]
    ...
    if (
        using_observation
        and "observation" in env.observation_spec
        and env.observation_spec["observation"].dtype == torch.float64
    ):
        ...
        if transforms is not None:
            ...
            if isinstance(transforms, Compose):
                if not any([isinstance(tr, DoubleToFloat) for tr in transforms]):
                    transforms = Compose(
                        DoubleToFloat(in_keys=["observation"]), *transforms
                    )
            else:
...
def wrap_environment(env: EnvBase, wrap_cfg: OmegaConf) -> EnvBase:
    wrapper_class = getattr(cwrappers, wrap_cfg.type)
    return wrapper_class(env, **wrap_cfg.args)


def get_env_action_dim(env: EnvBase) -> int:
...
def get_env_obs_dim(env: EnvBase):
