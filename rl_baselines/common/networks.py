...
def mlp_builder(
    ...
    if activation_out_info is not None:
        if isinstance(activation_out_info, str):
            ...
            kwargs_out = (
                activation_out_info.kwargs
                if activation_out_info.kwargs is not None
                else {}
            )
...
def init_final_linear_layer(model: nn.Sequential, min_value: float, max_value: float):
...
def cnn_dqn(out_dim: int, layers: List[Dict]) -> nn.Sequential:
    layers_modules = []
    for idx, (layer_dict) in enumerate(layers):
        for layer_type, layer_props in layer_dict.items():
            ...
            if (
                idx == (len(layers) - 1)
                and module_class == nn.Linear
                and layer_props["out_features"] is None
            ):
