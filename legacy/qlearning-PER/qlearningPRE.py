...
class MemoryReplayPRE:
    ...
    def sample(self, batch_sz, device=torch.device("cpu")):
        b_idx, isw = (
            np.empty((batch_sz,), dtype=np.int32),
            np.empty((batch_sz), dtype=np.float32),
        )
        b_memory = list()
        segment = self.tree.total_priority / batch_sz

        # Annealing beta from 0.4 to 1.0 exponent of important sampling
        self.beta = np.min([1.0, self.beta + self.beta_inc_rate])

        min_prob = (
            np.min(self.tree.tree[-self.tree.capacity :]) / self.tree.total_priority
        )
        ...
        do = (
            torch.from_numpy(np.vstack([float(e[4]) for e in b_memory]))
            .float()
            .to(device)
        )
...
class MemoryReplay:
    ...
    def sample(self, batch_sz, device):
        ...
        do = (
            torch.from_numpy(np.vstack([float(e[4]) for e in batch])).float().to(device)
        )
    ...
    for episode in range(NUMBER_EPISODES):
        ...
        for t in range(EPISODE_MAX_LEN):
            ...
            if True:
                ...
                if FramesCounter > REPLAY_START_SIZE:
                    ...
                    with torch.no_grad():
                        Qtarg = batch[1].squeeze(1) + GAMMA * target_qlearner(
                            batch[3]
                        ).max(dim=1)[0].detach() * (1.0 - batch[4].squeeze(1))

                    Qpred = torch.gather(
                        qlearner(batch[0]), 1, batch[2].long()
                    ).squeeze(1)
                    ...
                    memory_replay.batch_update(
                        b_index, np.asarray(newpriorities.to("cpu"))
                    )

                    e_greed = max(
                        EPSILON_END, e_greed - DECAY_RATE
                    )  # CAMBIAR A EXPONENTIAL DECAY
                    ...
                    if ((episode + 1) % 10 == 0) & (t % 10 == 0):
                        print(
                            "[{}, {}] \t\t {}\t\t{}".format(
                                episode + 1, t, loss_print, e_greed
                            )
                        )
        ...
        if (FramesCounter < REPLAY_START_SIZE) & (FramesCounter % 1000 == 0):
            ...
            if (episode + 1) % 100 == 0:
                ...
                print(
                    "**************************\nMean {} Last Score>\t{}".format(
                        len(list_reward), meanrw
                    )
                )
...
def testqlearner(env, name, n_episodes):
    ...
    for ep in range(1, n_episodes + 1):
        ...
        for t in range(EPISODE_MAX_LEN):
            ...
            with torch.no_grad():
                qvalues = qlearner(
                    torch.tensor(ob, dtype=torch.float32, device=device).unsqueeze(0)
                )
                action = qvalues.argmax().item()
                # print(action)
