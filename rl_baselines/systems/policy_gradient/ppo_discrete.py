    ...
    def training_step(self, batch, batch_idx):
        ...
        buffer = ReplayBuffer(
            storage=LazyTensorStorage(
                max_size=n_rollouts * max_trajectory_length, device=self.device
            ),
        ...
        for iroll in range(n_rollouts):
            with torch.no_grad():
                episode_data = self.env.rollout(
                    max_trajectory_length, policy=self.policy, auto_cast_to_device=True
                )
            ...
            episode_data["advantage"] = (
                episode_data["advantage"] - episode_data["advantage"].mean()
            ) / (episode_data["advantage"].std() + 1e-8)
            """
            advantages = [
        ...
        # import pdb;pdb.set_trace()
        nbatches = math.ceil(
            self.cfg.trainer.epochs_per_episode
            * len(buffer)
