...
def plot_rgb_stack(frame: torch.Tensor, stack: torch.Tensor, next_stack: torch.Tensor):
...
def check_replay_buffer(
    replay_buffer, key1, key2, batch_size: int, done_offset: int = 0
):
    sz = len(replay_buffer)
    dones_trues = (
        replay_buffer["next", "done"].squeeze(-1).nonzero().squeeze(-1).cpu().tolist()
    )
    index_done = 0
    total_diff = 0.0
    it = done_offset
