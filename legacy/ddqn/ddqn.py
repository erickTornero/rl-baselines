            ...
            if GeneralCounter % 4 == 0:
                if random.random() < e_greed:
                    ...
                    with torch.no_grad():
                        qvalues = dqn(
                            torch.tensor(
                                xin, dtype=torch.float32, device=device
                            ).unsqueeze(0)
                        )
                ...
                if FramesCounter > REPLAY_START_SIZE:
                    ...
                    Qtarg = batch[1].squeeze(1) + GAMMA * target_dqn(batch[3]).max(
                        dim=1
                    )[0].detach() * (1.0 - batch[4].squeeze(1))
        ...
        if episode % 5 == 0:
            print(
                "Mean {} Last Score>\t{}".format(
                    len(list_reward), sum(list_reward) / len(list_reward)
                )
            )
